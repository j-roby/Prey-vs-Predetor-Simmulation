    ...
    if (prob <= 0.05) {
        while(1) {
            ...
            if (dir == true) {
                ...
                if ( (pos = is_wolf_empty((nextField[newI][newJ]))) != -1) {
                    newWorld->wolves++;
                    nextField[i][j].wolf[wolfPos]->hunger = nextField[i][j].wolf[wolfPos]->hunger /2;
                    nextField[i][j].wolf[wolfPos]->reproduction = newWorld->setup.wolfMate;
                    nextField[i][j].wolf[wolfPos]->rest = true;
                    nextField[newI][newJ].wolf[pos] = malloc(sizeof(Predator));
                    nextField[newI][newJ].wolf[pos]->hunger = random_value(newWorld->setup.wolfGain*2,newWorld->setup.wolfGain /2);
                    nextField[newI][newJ].wolf[pos]->reproduction = newWorld->setup.wolfMate;
    ...
    if (prob <= 0.04) {
        while(1) {
            ...
            if (dir == true) {
                ...
                if ( (pos = is_sheep_empty((nextField[newI][newJ]))) != -1) {
                    newWorld->herd++;
                    nextField[i][j].sheep[sheepPos]->hunger = nextField[i][j].sheep[sheepPos]->hunger /2;
                    nextField[i][j].sheep[sheepPos]->reproduction = newWorld->setup.sheepMate;
                    nextField[i][j].sheep[sheepPos]->rest = true;
                    nextField[newI][newJ].sheep[pos] = malloc(sizeof(Prey));
                    nextField[newI][newJ].sheep[pos]->hunger = random_value(newWorld->setup.sheepGain*2,newWorld->setup.sheepGain /2);
                    nextField[newI][newJ].sheep[pos]->reproduction = newWorld->setup.sheepMate;
        ...
        for(int j = 0; j < newWorld->size; j++) {
           ...
           if (newWorld->field[i][j].sheep[0] != NULL) {
            ...
            if (newWorld->field[i][j].sheep[1] != NULL) {
                ...
                nextField[i][j].sheep[1] = malloc(sizeof(Predator));
...
int main(int argc, char** argv) {
    ...
    if (newWorld.wolves+newWorld.herd > size*size*2)
